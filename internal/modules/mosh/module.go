...
// Teardown removes the host-level reachability changes Apply made: the
// abysslink-managed PATH symlink (macOS) and the mosh UDP-range firewall rule
// (Linux). Best-effort and idempotent per modules.Teardowner — a change already
// gone returns no item and no error.
//
// The macOS Application Firewall allow for mosh-server is intentionally LEFT in
// place: socketfilterfw stores the resolved (Cellar) binary path rather than the
// symlink path abysslink adds, so a precise, reliable removal is not possible —
// and an extra allowed binary is inert. The PATH symlink and the Linux UDP rule
// are removed because each matches exactly what Apply created.
func (m *Module) Teardown(ctx context.Context, dryRun bool) ([]string, error) {
	...
	if pl, ok := m.plat.(platform.PathLinker); ok {
		if serverPath, err := m.moshServerPath(ctx); err == nil {
			desc, derr := pl.RemoveFromSystemPath(ctx, serverPath, dryRun)
			if derr != nil {
				return removed, fmt.Errorf("mosh teardown: remove PATH symlink: %w", derr)
			}
			if desc != "" {
				removed = append(removed, "PATH symlink "+desc)
			}
		}
	}
	...
	if rf, ok := fw.(platform.RangeFirewall); ok {
		if status, _ := fw.Status(ctx); status == "enabled" {
			if allowed, _ := rf.IsPortRangeAllowed(ctx, moshUDPLow, moshUDPHigh, "udp"); allowed {
				desc, derr := rf.RemovePortRange(ctx, moshUDPLow, moshUDPHigh, "udp", dryRun)
				if derr != nil {
					return removed, fmt.Errorf("mosh teardown: remove UDP range: %w", derr)
				}
				if desc != "" {
