...
func (f *fakeDarwinPlatform) RemoveFromSystemPath(_ context.Context, p string, dryRun bool) (string, error) {
	if !f.onSystemPath {
		return "", nil // nothing managed to remove
	}
	if !dryRun {
		f.removeLinkCalls++
	}
...
func (f *fakeRangeFirewall) RemovePortRange(_ context.Context, lo, hi int, proto string, dryRun bool) (string, error) {
	if !dryRun {
		f.removeRangeCalls++
	}
	return fmt.Sprintf("host-firewall allow %d:%d/%s", lo, hi, proto), nil
}
...
func TestTeardown_Darwin_RemovesSymlink(t *testing.T) {
	r := shell.NewMockRunner(brewPrefixCall())
	plat := &fakeDarwinPlatform{fw: &fakeAppFirewall{status: "enabled"}, onSystemPath: true}
	m := New(modules.Deps{Cfg: enabledCfg(), Runner: r, Platform: plat})

	removed, err := m.Teardown(context.Background(), false)
	require.NoError(t, err)
	require.Len(t, removed, 1)
	assert.Contains(t, removed[0], "PATH symlink")
	assert.Equal(t, 1, plat.removeLinkCalls)
}

func TestTeardown_DryRun_ReportsWithoutRemoving(t *testing.T) {
	r := shell.NewMockRunner(brewPrefixCall())
	plat := &fakeDarwinPlatform{fw: &fakeAppFirewall{status: "enabled"}, onSystemPath: true}
	m := New(modules.Deps{Cfg: enabledCfg(), Runner: r, Platform: plat})

	removed, err := m.Teardown(context.Background(), true)
	require.NoError(t, err)
	require.Len(t, removed, 1, "dry-run still reports the item")
	assert.Zero(t, plat.removeLinkCalls, "dry-run must not mutate")
}

func TestTeardown_Linux_RemovesRangeWhenPresent(t *testing.T) {
	r := shell.NewMockRunner() // no brew on linux path
	fw := &fakeRangeFirewall{status: "enabled", allowed: true}
	m := New(modules.Deps{Cfg: enabledCfg(), Runner: r, Platform: &fakeLinuxPlatform{fw: fw}})

	removed, err := m.Teardown(context.Background(), false)
	require.NoError(t, err)
	require.Len(t, removed, 1)
	assert.Equal(t, 1, fw.removeRangeCalls)
}

func TestTeardown_Linux_NoRuleNoOp(t *testing.T) {
	r := shell.NewMockRunner()
	fw := &fakeRangeFirewall{status: "enabled", allowed: false} // range not present
	m := New(modules.Deps{Cfg: enabledCfg(), Runner: r, Platform: &fakeLinuxPlatform{fw: fw}})

	removed, err := m.Teardown(context.Background(), false)
	require.NoError(t, err)
	assert.Empty(t, removed)
	assert.Zero(t, fw.removeRangeCalls)
}
