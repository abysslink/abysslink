...
Phases 22, 23, 23.1, 23.2, 24, 25, 26 (7 phases, 29 plans) — closes the 2026-06-03 full security audit (11 agents) network/dependency/doctor/audit/DoS findings. All 18 requirements `[x]` verified.
...
- **Phase 26** (post-ship fold-in) — gated 8-stage init journey + first-run fixes: openURL `shell.LookPath` probe, Stage 2 autoYes duplication, pmset annotated-output parse, sudo via `RunInteractive` (one password per run), power short-circuit, terminal-state restore. Verification 25/25, human UAT approved.
...
### Phase 26: Init journey gating & first-run fixes (post-ship fold-in)

**Goal:** `abysslink init` becomes a genuinely gated interactive wizard: each journey stage pauses for user confirmation, stages 3–6 offer to actually run `up --apply` / `lock init --apply` / `enroll phone` / `doctor` (instead of printing the command and scrolling past), a new ACL stage guides `abysslink acl push --apply`, and three first-run bugs are fixed — (1) openURL macOS probe uses `open --version` which exits 1 so the browser never opens yet "Browser opened" is printed; (2) journey Stage 2 hardcodes autoYes=true so sudo mutations run unprompted and duplicate init RunE work; (3) checkACSleepDisabled pmset parser requires exactly-2-field "sleep 0" lines, but `pmset -g` emits trailing annotations, so `sudo pmset` re-runs every init. Headless paths (--yes/--json/non-TTY) stay non-blocking; --resume keeps working via journey-state.json.
**Requirements**: phase-internal (5/5 success criteria; see incremental audit)
**Depends on:** Phase 25
**Plans:** 3/3 plans complete
**Note:** Executed after the pushed `v3.0.1` tag (17 commits post-tag); folded into this milestone per incremental audit `v3.0.1-MILESTONE-AUDIT-phase-26.md`. Verification 25/25 (re-verification after gap-closure 26-03), human UAT approved.
